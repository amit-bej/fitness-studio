...
user side, while ``psycopg`` by default will bind them server side.

It's possible to configure ``psycopg`` to do user side binding by
specifying the ``cursor_factory`` to be ``userCursor`` when creating
the engine::

    from psycopg import userCursor

    user_side_engine = create_engine(
        "postgresql+psycopg://...",
        connect_args={"cursor_factory": userCursor},
    )

Similarly when using an async engine the ``AsyncuserCursor`` can be
specified::

    from psycopg import AsyncuserCursor

    user_side_engine = create_async_engine(
        "postgresql+psycopg://...",
        connect_args={"cursor_factory": AsyncuserCursor},
    )

.. seealso::

    `user-side-binding cursors <https://www.psycopg.org/psycopg3/docs/advanced/cursors.html#user-side-binding-cursors>`_
...
class PGDialect_psycopg(_PGDialect_common_psycopg):
    ...
    def create_connect_args(self, url):
        ...
        if self.user_encoding is not None:
            cparams["user_encoding"] = self.user_encoding
