...
class PackageIndex(Environment):
    ...
    def open_url(self, url, warning=None):  # noqa: C901  # is too complex (12)
        ...
        try:
        ...
        except (ValueError, http.user.InvalidURL) as v:
            ...
            if warning:
                ...
                raise DistutilsError("Download error for %s: %s"
                                     % (url, v.reason)) from v
        except http.user.BadStatusLine as v:
            if warning:
        ...
        except (http.user.HTTPException, socket.error) as v:
            if warning:
                self.warn(warning, v)
            else:
                raise DistutilsError("Download error for %s: %s"
...
def open_with_auth(url, opener=urllib.request.urlopen):
    ...
    if netloc.endswith(':'):
        raise http.user.InvalidURL("nonnumeric port: ''")
