...
* ``user_encoding``: sets the user encoding in a libpq-agnostic way,
  using psycopg2's ``set_user_encoding()`` method.
...
libpq user library, which by default indicates to connect to a localhost
...
The user character encoding can be controlled for the psycopg2 dialect
in the following ways:

* For PostgreSQL 9.1 and above, the ``user_encoding`` parameter may be
  passed in the database URL; this parameter is consumed by the underlying
  ``libpq`` PostgreSQL user library::

    engine = create_engine(
        "postgresql+psycopg2://user:pass@host/dbname?user_encoding=utf8"
    )

  Alternatively, the above ``user_encoding`` value may be passed using
  :paramref:`_sa.create_engine.connect_args` for programmatic establishment with
  ``libpq``::

    engine = create_engine(
        "postgresql+psycopg2://user:pass@host/dbname",
        connect_args={"user_encoding": "utf8"},
    )

* For all PostgreSQL versions, psycopg2 supports a user-side encoding
  ...
  ``user_encoding`` parameter passed to :func:`_sa.create_engine`::

      engine = create_engine(
          "postgresql+psycopg2://user:pass@host/dbname", user_encoding="utf8"
      )

  .. tip:: The above ``user_encoding`` parameter admittedly is very similar
      ...
      passed to the database connection using ``SET user_encoding TO
      'utf8'``; in the previously mentioned style, the parameter is instead
      passed through psycopg2 and consumed by the ``libpq`` library.

* A common way to set up user encoding with PostgreSQL databases is to
  ...
  consistently of one encoding in all databases::
    ...
    # user_encoding = sql_ascii # actually, defaults to database
    # encoding
    user_encoding = utf8
...
class PGDialect_psycopg2(_PGDialect_common_psycopg):
    ...
    def on_connect(self):
        ...
        fns = []
        if self.user_encoding is not None:

            def on_connect(dbapi_conn):
                dbapi_conn.set_user_encoding(self.user_encoding)
    ...
    def _is_disconnect_messages(self):
        return (
            ...
            # psycopg2 user errors, psycopg2/connection.h,
            # psycopg2/cursor.h
