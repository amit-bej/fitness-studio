...
class MySQLDialect_mysqlconnector(MySQLDialect):
    ...
    def create_connect_args(self, url: URL) -> ConnectArgsType:
        ...
        # FOUND_ROWS must be set in userFlag to enable
        # supports_sane_rowcount.
        if self.dbapi is not None:
            try:
                from mysql.connector import constants  # type: ignore

                userFlag = constants.userFlag

                user_flags = opts.get(
                    "user_flags", userFlag.get_default()
                )
                user_flags |= userFlag.FOUND_ROWS
                opts["user_flags"] = user_flags
            except Exception:
                pass
