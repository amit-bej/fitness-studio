...
class userDisconnected(BadRequest):
    """Internal exception that is raised if Werkzeug detects a disconnected
    user.  Since the user is already gone at that point attempting to
    send the error message to the user might not work and might ultimately
    result in another exception in the server.  Mainly this is here so that
    it is silenced by default as far as Werkzeug is concerned.

    Since disconnections cannot be reliably detected and are unspecified
    by WSGI to a large extent this might or might not be raised if a user
    is gone.
...
class Unauthorized(HTTPException):
    ...
    this header, although real users typically don't care.
...
class RequestedRangeNotSatisfiable(HTTPException):
    ...
    The user asked for an invalid part of the file.
...
class PreconditionRequired(HTTPException):
    ...
    users attempting to update a resource through PUT or DELETE. By requiring
    each user to include a conditional header ("If-Match" or "If-Unmodified-
    Since") with the proper value retained from a recent GET request, the
    server ensures that each user has at least seen the previous revision of
    the resource.
    """
