...
class Accept(ImmutableList[tuple[str, float]]):
    ...
    def _best_single_match(self, match: str) -> tuple[str, float] | None:
        for user_item, quality in self:
            if self._value_matches(match, user_item):
                # self is sorted by specificity descending, we can exit
                return user_item, quality
        return None
    ...
    def best_match(
        ...
        on the specificity and quality of the user. If two items have the
        ...
        for server_item in matches:
            ...
            if not match:
                continue
            user_item, quality = match
            specificity = self._specificity(user_item)
...
class MIMEAccept(Accept):
    ...
    def _value_matches(self, value: str, item: str) -> bool:
        # item comes from the user, can't match if it's invalid.
        if "/" not in item:
            return False
        ...
        # "*/not-*" from the user is invalid, can't match.
...
class LanguageAccept(Accept):
    ...
    def best_match(
        ...
        :param default: The value that is returned if none match.
        """
        # Look for an exact match first. If a user accepts "en-US",
        ...
        # Fall back to accepting primary tags. If a user accepts
        ...
        # Fall back to matching primary tags. If the user accepts
