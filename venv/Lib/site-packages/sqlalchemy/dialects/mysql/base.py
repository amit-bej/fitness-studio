...
MySQLdb, MySQL-user, MySQL-Connector Python, and PyMySQL.   Using it,
...
Server-side cursor support is available for the mysqluser, PyMySQL,
...
Most MySQL / MariaDB DBAPIs offer the option to set the user character set for
...
This charset is the **user character set** for the connection.  Some
...
charset is preferred, if supported by both the database as well as the user
DBAPI, as in::
...
This warning is due to the fact that the MySQL user library is attempting to
...
use of mysqluser or PyMySQL (both of which are recommended).  Add the query
...
both Servers and users. To change the ``sql_mode`` for a given application, a
...
SQLAlchemy MySQL dialects always add the ``constants.user.FOUND_ROWS``
