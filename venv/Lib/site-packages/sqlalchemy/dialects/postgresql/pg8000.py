...
PostgreSQL ``user_encoding`` parameter; by default this is the value in
the ``postgresql.conf`` file, which often defaults to ``SQL_ASCII``.
Typically, this can be changed to ``utf-8``, as a more useful default::

    # user_encoding = sql_ascii # actually, defaults to database encoding
    user_encoding = utf8

The ``user_encoding`` can be overridden for a session by executing the SQL:

.. sourcecode:: sql

    SET user_ENCODING TO 'utf8';

SQLAlchemy will execute this SQL on all new connections based on the value
passed to :func:`_sa.create_engine` using the ``user_encoding`` parameter::

    engine = create_engine(
        "postgresql+pg8000://user:pass@host/dbname", user_encoding="utf8"
    )
...
or does not match the host name (as seen from the user), it may also be
...
class PGDialect_pg8000(PGDialect):
    ...
    def __init__(self, user_encoding=None, **kwargs):
    ...
    def _set_user_encoding(self, dbapi_connection, user_encoding):
        cursor = dbapi_connection.cursor()
        cursor.execute(
            f"""SET user_ENCODING TO '{
    ...
    def on_connect(self):
        ...
        if self.user_encoding is not None:
            ...
            def on_connect(conn):
                self._set_user_encoding(conn, self.user_encoding)
