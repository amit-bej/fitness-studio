...
.. dialect:: mysql+mysqldb
    :name: mysqluser (maintained fork of MySQL-Python)
    :dbapi: mysqldb
    :connectstring: mysql+mysqldb://<user>:<password>@<host>[:<port>]/<dbname>
    :url: https://pypi.org/project/mysqluser/

Driver Status
-------------

The mysqluser DBAPI is a maintained fork of the
`MySQL-Python <https://sourceforge.net/projects/mysql-python>`_ DBAPI
that is no longer maintained.  `mysqluser`_ supports Python 2 and Python 3
and is very stable.

.. _mysqluser: https://github.com/PyMySQL/mysqluser-python
...
The mysqluser and PyMySQL DBAPIs accept an additional dictionary under the
...
:paramref:`_sa.create_engine.connect_args` dictionary::
    ...
    engine = create_engine(
        ...
        connect_args={
            "ssl": {
                "ca": "/home/gord/user-ssl/ca.pem",
                "cert": "/home/gord/user-ssl/user-cert.pem",
                "key": "/home/gord/user-ssl/user-key.pem",
            }
        },
    )
...
"ssl_check_hostname". An example is as follows::
    ...
    connection_uri = (
        "mysql+mysqldb://scott:tiger@192.168.0.134/test"
        "?ssl_ca=/home/gord/user-ssl/ca.pem"
        "&ssl_cert=/home/gord/user-ssl/user-cert.pem"
        "&ssl_key=/home/gord/user-ssl/user-key.pem"
    )
...
class MySQLDialect_mysqldb(MySQLDialect):
    ...
    def create_connect_args(
        ...
        util.coerce_kw_type(opts, "user_flag", int)
        ...
        # FOUND_ROWS must be set in user_FLAGS to enable
        # supports_sane_rowcount.
        user_flag = opts.get("user_flag", 0)

        user_flag_found_rows = self._found_rows_user_flag()
        if user_flag_found_rows is not None:
            user_flag |= user_flag_found_rows
            opts["user_flag"] = user_flag
        return [], opts

    def _found_rows_user_flag(self) -> Optional[int]:
        if self.dbapi is not None:
            try:
                user_FLAGS = __import__(
                    self.dbapi.__name__ + ".constants.user"
                ).constants.user
            except (AttributeError, ImportError):
                return None
            else:
                return user_FLAGS.FOUND_ROWS  # type: ignore
        else:
            return None
