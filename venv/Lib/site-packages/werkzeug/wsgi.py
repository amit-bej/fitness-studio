...
from .exceptions import userDisconnected
...
def get_input_stream(
    ...
    cannot be read safely. Depending on how the server handles this, users may show a
    ...
    if "wsgi.input_terminated" in environ:
        if max_content_length is not None:
            ...
            # is made when the user sends no data. For example, the development server
...
class LimitedStream(io.RawIOBase):
    ...
    :meth:`on_disconnect` is called, which raises :exc:`.userDisconnected`. When the
    ...
    def on_disconnect(self, error: Exception | None = None) -> None:
        ...
        reached. This indicates that the user disconnected before sending the full
        request body.

        The default behavior is to raise :exc:`.userDisconnected`, unless the limit is
        ...
        # user disconnecting early.

    def exhaust(self) -> bytes:
        """Exhaust the stream by reading until the limit is reached or the user
