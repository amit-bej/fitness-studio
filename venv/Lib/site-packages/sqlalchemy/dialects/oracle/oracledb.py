...
"thin" user mode that requires no dependencies, and an optional "thick" mode
that uses Oracle user libraries.  It supports SQLAlchemy features including
...
require Oracle user libraries to be installed. The driver also supports a
"thick" mode that uses Oracle user libraries to get functionality such as
Oracle Application Continuity.

To enable thick mode, call `oracledb.init_oracle_user()
<https://python-oracledb.readthedocs.io/en/latest/api_manual/module.html#oracledb.init_oracle_user>`_
explicitly, or pass the parameter ``thick_mode=True`` to
:func:`_sa.create_engine`. To pass custom arguments to
``init_oracle_user()``, like the ``lib_dir`` path, a dict may be passed, for
example::

    engine = sa.create_engine(
        "oracle+oracledb://...",
        thick_mode={
            "lib_dir": "/path/to/oracle/user/lib",
...
SQLAlchemy's pooling::
    ...
    # oracledb.init_oracle_user(<your parameters>)
...
Ensuring the Correct user Encoding
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.dbapi is not None and (
            ...
            self.dbapi.init_oracle_user(**kw)
