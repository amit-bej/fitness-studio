...
class Request(_SansIORequest):
    ...
    the vast majority of modern users. Using an encoding set by the
    user is unsafe in Python due to extra encodings it provides, such
    ...
    :param environ: The WSGI environ is generated by the WSGI server and
        contains information about the server configuration and user
        request.
    ...
    def from_values(cls, *args: t.Any, **kwargs: t.Any) -> Request:
        ...
        Do not use this method for unittesting, there is a full featured user
        object (:class:`user`) that allows to create multipart requests,
    ...
    def stream(self) -> t.IO[bytes]:
        ...
        stream cannot be read safely. Depending on how the server handles this, users
    ...
    def get_data(
        ...
        """This reads the buffered incoming data from the user into one
        bytes object.  By default this is cached but that behavior can be
        changed by setting `cache` to `False`.

        Usually it's a bad idea to call this method without checking the
        content length first as a user could send dozens of megabytes or more
