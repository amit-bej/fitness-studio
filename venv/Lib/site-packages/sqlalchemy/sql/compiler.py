...
class _InsertManyValues(NamedTuple):
    ...
    that would link to the user side "sentinel" values for that row, which
    we can use to match up parameter sets to result rows.

    This is only present if sentinel_columns is present and the INSERT
    statement actually refers to user side values for these sentinel
    columns.
    ...
    def visit_insert(
        ...
        if returning_cols:
            ...
            if add_sentinel_cols is not None:
                ...
                # we will have a user side value for the sentinel in each
                # parameter set.
                ...
                if named_sentinel_params is None:
                    # if we are not going to have a user side value for
